...
# library_path = os.path.join(current_dir, "/home/lzz/Projects/DBGPS_Python/kISS/build")  # 假设动态链接库在 build 目录下
...
#                         self.pNode['a'] = path[len(path) - self.kmer_len:len(path)]
...
#                     if(len(tmpPaths)==0):
#                         currK -= 1
#                 if(self.mid_k <= currK < self.kmer_len):
#                     while(len(tmpPaths) == 0 and currK > self.mid_k):
#                         for path in self.pathsA:
#                             edgeScore = {'A': 0, 'T': 0, 'G': 0, 'C': 0}
#                             pkmer = path[len(path)-currK:len(path)]
#                             res = self.tree.search_prefix_flag(dna_to_numbers(pkmer[1:]),0,len(pkmer)-1)
#                             if res is None:
#                                 break
#                             for pos in res[1:]:
...
#     #             if prefix not in self.unitigs_prefix[k]:
#     #                 self.unitigs_prefix[k][prefix] = []
#     #             self.unitigs_prefix[k][prefix].append(unitig)
                
#     #             if suffix not in self.unitigs_suffix[k]:
#     #                 self.unitigs_suffix[k][suffix] = []
#     #             self.unitigs_suffix[k][suffix].append(unitig)
...
#             # rc_prefix = rc_unitig[:self.kmer_len-1]  # 反向互补序列的前缀
#             # rc_suffix = rc_unitig[-self.kmer_len+1:]  # 反向互补序列的后缀
...
#     #             fragments.append(unitig[prev:pos+self.kmer_len-1])
#     #             if len(unitig[prev:pos+self.kmer_len-1]) == self.kmer_len-1:
#     #                 print('ddd')
#     #             prev = pos
#     #         fragments.append(unitig[prev:])
#     #         if len(unitig[prev:]) == self.kmer_len-1:
#     #                 print('ddd')
#     #         fragments_cov = self.compute_fragments_cov(fragments)

#     #         first_cov = fragments_cov[fragments[0]]
#     #         last_cov = fragments_cov[fragments[len(fragments)-1]]
#     #         if len(breakpoints) > 0:
#     #             del fragments_cov[fragments[0]]
#     #             del fragments_cov[fragments[len(fragments)-1]]
#     #             fragments_cov[ex_left+fragments[0]] = (s_left+first_cov*(len(fragments[0])-self.kmer_len+1))/(len(ex_left)+len(fragments[0])-self.kmer_len+1)
#     #             fragments_cov[fragments[len(fragments)-1]+ex_right] = (s_right+last_cov*(len(fragments[len(fragments)-1])-self.kmer_len+1))/(len(ex_right)+len(fragments[len(fragments)-1])-self.kmer_len+1)
#     #         else:
#     #             del fragments_cov[fragments[0]]
#     #             fragments_cov[ex_left+fragments[0]+ex_right] = (s_right+s_left+first_cov*(len(fragments[0])-self.kmer_len+1))/(len(ex_right)+len(ex_left)+len(fragments[0])-self.kmer_len+1)
#     #         self.new_unitigs.update(fragments_cov)
...
#                                     else:
#                                         break_info[base] = (1,rc_res[0])
#                                     # confidence = self.compute_path_confidence(node+base,pathsA[node][0]+(currK/self.kmer_len)*count,unitig)
#                                     # if max_confidence == 0:
#                                     #     max_confidence_path = node+base
#                                     #     max_count = pathsA[node][0]+(currK/self.kmer_len)*count
#                                     #     max_confidence = confidence
#                                     #     max_break_info = break_info
#                                     # else:
#                                     #     if confidence > max_confidence:
...
#                                     else:
#                                         break_info[base] = (1,rc_res[0])
#                                     # confidence = self.compute_path_confidence(base+node,pathsA[node][0]+(currK/self.kmer_len)*count,unitig)
#                                     # if max_confidence == 0:
#                                     #     max_confidence_path = base+node
#                                     #     max_count = pathsA[node][0]+(currK/self.kmer_len)*count
#                                     #     max_confidence = confidence
#                                     #     max_break_info = break_info
#                                     # else:
