...
> 说明：当前代码中主训练文件来自 `Pasted code.py`，建议正式放入仓库时重命名为 `train.py`；`snp.py` 底部存在直接执行的示例代码，建议移动到 `scripts/build_graph.py` 或加上 `if __name__ == "__main__":`，避免在其他模块 import 时自动运行。
...
├── README.md
├── requirements.txt
├── train.py                         # 原 Pasted code.py：训练、验证、测试、保存 read score
├── gate_gcn.py                      # 主要模型：GraphGatedGCNModel / FeatureGate
├── snp.py                           # SNP 特征提取、read-read 图构建、DGL 图保存
├── simulate.py                      # 合成数据生成：VCF 过滤、haplotype consensus、PBSIM3 模拟、下采样
├── evaluate_saved_graph.py          # read-level score -> SNP-level phased VCF
├── dump_reads_by_score.py           # read-level score -> hap0/hap1/ambiguous read list
├── estimate_bam_coverage.py         # BAM 覆盖度估计与 samtools 下采样比例建议
├── globals.py                       # 项目全局变量，需保留
├── hyperparameters.py               # 训练/测试参数入口，需保留 get_hyperparameters()
├── models.py                        # 模型入口，需暴露 GraphGatedGCNModel
├── layers.py                        # GNN layer 与 ScorePredictor
├── utils.py                         # add_positional_encoding、set_seed 等工具函数
├── data/
│   ├── train_graphs/
│   ├── val_graphs/
│   └── test_graphs/
...
- `simulate.py`：根据样本 phased VCF 构造 hap1/hap2 consensus，并用 PBSIM3 模拟 ONT reads；也提供 minimap2 比对、PAF 构建和 samtools 下采样逻辑。
- `snp.py`：将 SNP 位点投影到 reads 上，生成节点 SNP 特征 `[n_het_cov, ref_frac, alt_frac, conflict_frac]`，并在 PAF 边上生成边 SNP 特征 `[n_shared_het, n_same_allele, n_diff_allele, allele_consistency]`；随后结合 read 长度、overlap 长度和相似度构建 DGL graph。
- `gate_gcn.py`：定义带 feature-level gate 的 `GraphGatedGCNModel`，用于学习节点和边特征的重要性，并输出 read-level haplotype score。
- `train.py`：读取 `.bin` 格式 DGL 图，进行训练、验证或测试；测试阶段将每个 read 的 haplotype-1 概率写入 `graph.ndata["score"]`，并保存预测后的 graph。
- `evaluate_saved_graph.py`：读取测试后的 graph score，在 SNP 位点上按 hap0/hap1 read 支持数投票，输出 predicted phased VCF。
- `dump_reads_by_score.py`：按阈值将 reads 分成 hap0、hap1 和 ambiguous 三类，便于后续分 haplotype 组装。
- `estimate_bam_coverage.py`：估计 BAM 平均覆盖度，并输出生成 5x、10x、15x、30x 数据的 samtools 下采样命令。
...
reads_to_ref_or_contig.bam(.bai)      # reads 与 reference/contig 的比对
reads.paf                             # read-read overlap PAF
sample.id2name                        # 数字 read id 到原始 read name 的映射
sample_id.fasta                       # read header 已转为数字 id 的 FASTA
```

### 4.2 Real SNP VCF available
...
### 4.3 No real SNP VCF: call SNPs with PECAT + minimap2 + Clair3
...
## 5. Prepare reads and alignments

### 5.1 Rename reads to numeric IDs
...
### 5.3 Reads-to-reference or reads-to-contig BAM
...
## 6. Build SNP features and DGL graph

### 6.1 SNP feature definitions
...
### 6.2 Example graph construction script
...
## 8. Testing and read-level score output
...
## 10. Read-level score to SNP-level phased VCF
...
## 11. Evaluation: switch rate/error, mismatch/Hamming error, N50/AN50
...
### 11.1 Prepare per-chromosome VCFs
...
## 12. Haplotype-specific assembly with hifiasm

模型分相后，可以根据 read score 将 reads 划分为 hap0/hap1，然后分别组装。

### 12.1 Extract haplotype-specific reads
...
### 12.2 hifiasm assembly commands
...
OUT_ROOT = BASE_DIR / "ralphi_ont_synth"
SAMPLES = ["HG00142"]
...
## 14. Coverage estimation and downsampling
